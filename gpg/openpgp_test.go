...
func Test_ConvertLocalSignerToPgpPrivateKey_RSA(t *testing.T) {
	now := time.Now().UTC()
	crt, pvk, err := testca.MakeSelfCertRSA(720)
	require.NoError(t, err)
...
func Test_ConvertLocalSignerToPgpPrivateKey_ECDSA(t *testing.T) {
	now := time.Now().UTC()
	crt, pvk, err := testca.MakeSelfCertECDSA(720)
	require.NoError(t, err)
...
func Test_ConvertPemToPgpPrivateKey(t *testing.T) {
	now := time.Now().UTC()
	_, keyPem, err := testca.MakeSelfCertRSAPem(720)
	require.NoError(t, err)
	_, err = ConvertPemToPgpPrivateKey(now, keyPem)
	require.NoError(t, err)

	_, keyPem, err = testca.MakeSelfCertECDSAPem(720)
	require.NoError(t, err)
	_, err = ConvertPemToPgpPrivateKey(now, keyPem)
	require.NoError(t, err)
